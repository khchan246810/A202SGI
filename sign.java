...
public class sign extends AppCompatActivity {
    ...
    private boolean CheckAllFields() {
        if (TextUtils.isEmpty(nameInput.getText().toString())) {
            Toast.makeText(sign.this, "Fill up the fields. For phone number, must equal to 10 digits. Try again.", Toast.LENGTH_SHORT).show();
            return false;
        } else {
        ...
        if (!resetPasswordInput.getText().toString().equals(passwordInput.getText().toString())) {
            Toast.makeText(sign.this, "Passwords do not matched.", Toast.LENGTH_SHORT).show();
            return false;
        } else {
            resetPasswordInput.setError(null); // Clear the error
        }
        ...
        if (!checkbox.isChecked()) {
            Toast.makeText(sign.this, "Please agree to the terms and conditions.", Toast.LENGTH_SHORT).show();
            return false;
        } else {
    ...
    private void signUpDatabase() {
        ...
        long result = dbManager.signUpData(name, phoneNumber, password);
